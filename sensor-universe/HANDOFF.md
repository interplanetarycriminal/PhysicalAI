...
**Added since v55: the ESP32 interface field set.** `data/schema.py` carries eleven optional interface fields (`iface_primary`, `v_min`, `v_max`, `logic_v`, `level_shift`, `addr_mode`, `cs_pins`, `i_peak_ua`, `rate_hz`, `esp32_driver`, `driver_status`) plus the `LEVEL_SHIFT`, `ADDR_MODE` and `DRIVER_STATUS` enums. The loader guarantees all eleven are present on every record — explicitly `None` where nothing is known. Nine are derived by `data/iface_derive.py` from text already in the record; `esp32_driver` and `driver_status` are **authored-only and now cover all 405 sensors** (every sensor has a `driver_status`; 394 name a specific driver, the other 11 are explicit `None` where no ESP32 driver exists), written as JSON and compiled into `data/enrich_iface.py` (the loader's fourth overlay) by `tools/gen_enrich_iface.py`. `verify_build.py` section 9 gates the field set and prints per-field coverage.
